...
| The VM lab | `vm-lab/` | disposable QEMU/KVM machine for measurements that write kernel-global or one-way state (the yama sweep lives here now); a container is not a substitute — it shares the host kernel |
