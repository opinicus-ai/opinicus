...
| 2026-09-01 | **M9 done.** [af-10] shipped: the gates are deterministic and CI-enforced. The e2e flake was SIGPIPE inside the assertion helper under pipefail (measured 5/3000 probes, needle present every time) — the helpers read without pipes now and the summary rides stderr; the monitor flake was the capability probe leaving half-dead children the next session reaped (9/60 under load) — the probe is shepherded to its reaped exit and foreign exits never enter the events; the three correlation rules are classified C and count-rules.py exits 0; every stale count reconciled; the benign corpus FAIL was a mid-M4 artifact nothing watched — the benign gate now runs one corpus mode inside every gate run. The complete gate passed **twenty consecutive times** on a clean CI runner (research/gates/, run 33475574579), e2e 54/54 in every run, and a synthetic red (inverted B2, PR #3) was **rejected**: required check failed, pull blocked. A second, rarer harness flake (IO-safety abort, 3 CI occurrences, gdb-caught shape committed) is filed as AF-a5xn5s with its instrument; the twenty-run evidence above is untouched by it |
| 2026-09-01 | **M10 done.** [af-11] shipped: the trust posture is enforceable. docs/THREAT-MODEL.md publishes the cooperative-agent contract with the phrasing table; SECURITY.md opens disclosure and the FP/FN templates route with labels; DATABASE_URL userinfo is masked at capture and traces/consent/outbox are 0600 (tests assert both negatives); deny.toml plus the dependency-contract test make the no-network property structural, cargo deny and cargo audit are green in the gate (63 deps, 0 advisories); the placeholder repository URL is real; docs/RELEASE.md defines the signing policy and CI proves it end to end — sign on one runner, verify on a second, corrupted-manifest and corrupted-byte both fail. Two independent adversarial reviewers (isolated agent instances) found no claim beyond the measured matrix after six flagged items were fixed |
| 2026-09-01 | **M11 done.** [af-12] shipped: the boundary gaps are closed or contract-named on measurements. io_uring is held at both calls and reported (the evade-15 zero-events gap closed as visibility); the decision is host-requirement with the numbers (36 ring calls in one npm ci) — a default deny would break the budget — and doctor names the machine's posture. The launch closes inherited descriptors beyond stdio (fds 6→3 measured, writes EBADF in every mode); in-tree inheritance and SCM_RIGHTS are named out-of-scope until a boundary tier. The hostile same-UID matrix ran under yama 0–3: attach/vmem/procmem succeed unsensed at 0 and are kernel-denied at 1; pidfd and external SIGKILL kill the monitor at every session-permitting scope — fail-closed (EXITKILL, durable trace, replay names the shape) but unsensed: accepted gaps in DECISIONS, no narrative upgrade; yama 2/3 refuse the launch itself (doctor fixed). docs/LANDLOCK-CONTRACT.md publishes the exact set contract with the 13/13 stress matrix; the /tmp credential carve-out is measured impossible with the mechanism named; e2e K9–K12 pin the writable-tree hole. The pointer-derived-facts invariant is type-level: a compile_fail doctest proves an allow cannot consume an advisory path, the mutation harness catches 1000/1000 flipped markings |
...
| 2026-09-01 | **M12 running.** The headless CI surface landed: `run --ci` (deterministic deny of every approval, no terminal opened, alpha banner kept, refused in combination with `--approve`), `run --summary <PATH>` (machine-readable session summary: every decision with rule id, evidence line and provenance chain; counts of denied/reported/quarantined/questions; the exit code), the exit-code contract table in `run --help` and README "The alpha", CLI integration tests (contract code, summary validity, replay consistency) and e2e section V — the live halves run in CI (this host is latched at yama ptrace_scope 3). The exit gate stays open: a 30-day self-pilot on a real CI job, zero merge-blocking false denials, <1h operator work per month. |
